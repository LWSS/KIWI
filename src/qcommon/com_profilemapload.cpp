...
void __cdecl ProfLoad_Activate()
{
    ...
    Com_Printf(CON_CHANNEL_PROFILE, S_COLOR_MAGENTA "Activating map load profiler\n");
}
...
int ProfLoad_PrintTree()
{
    ...
    for (profileIndex = 0; profileIndex < mapLoadProfile.profileEntryCount; ++profileIndex)
    {
        ...
        Com_Printf(CON_CHANNEL_PROFILE, S_COLOR_MAGENTA "%*c %s\n", 2 * entry->indent + 1, 32, rowText);
...
void ProfLoad_PrintHotSpots()
{
    ...
    Com_Printf(CON_CHANNEL_PROFILE, "\n\n" S_COLOR_MAGENTA "---------- Load time hot spots ----------\n");
    if (v14 > 16)
        v10 = 16;
    else
        v10 = v14;
    ...
    for (j = 0; j < v14; ++j)
    {
        ...
        Com_Printf(
            CON_CHANNEL_PROFILE,
            S_COLOR_MAGENTA "%s: %5.3f self, %5.3f file, %i hits\n",
            v11[j].label,
            (double)v18,
            (double)v17,
            v11[j].accessCount);
    }
    Com_Printf(CON_CHANNEL_PROFILE, "\n" S_COLOR_MAGENTA "Hot spot total time: %5.3f self, %5.3f file\n\n", (double)v15, (double)v16);
}
