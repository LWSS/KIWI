...
uint8_t *__cdecl CM_Hunk_AllocXModel(uint size)
{
    return Hunk_Alloc(size, "CM_Hunk_AllocXModel", 21);
}
uint8_t *__cdecl CM_Hunk_AllocXModelColl(uint size)
{
...
void __cdecl CM_LoadStaticModels()
{
    ...
    uint numStaticModels; // [esp+24h] [ebp-F8h]
...
void __cdecl CMod_LoadPlanes()
{
    ...
    uint planeIter; // [esp+14h] [ebp-18h]
    uint8_t bits; // [esp+1Fh] [ebp-Dh]
    char *in; // [esp+20h] [ebp-Ch] // KISAKTODO: assign struct type
    uint axisIter; // [esp+24h] [ebp-8h]
    uint count; // [esp+28h] [ebp-4h] BYREF
...
void CMod_LoadMaterials()
{
    ...
    uint matIndex; // [esp+8h] [ebp-8h]
    uint count; // [esp+Ch] [ebp-4h] BYREF
...
void CMod_LoadNodes()
{
    ...
    uint nodeIter; // [esp+10h] [ebp-Ch]
    int child; // [esp+14h] [ebp-8h]
    uint count; // [esp+18h] [ebp-4h] BYREF
...
void CMod_LoadLeafSurfaces()
{
    ...
    cm.leafsurfaces = (uint*)CM_Hunk_Alloc(4 * count, "CMod_LoadLeafSurfaces", 28);
...
void CMod_LoadCollisionTriangles()
{
    ...
    cm.triIndices = (ushort*)CM_Hunk_Alloc(2 * count, "CMod_LoadCollisionTriangles", 28);
    cm.triCount = count / 3;
...
void CMod_LoadCollisionEdgeWalkable()
{
    ...
    uint count; // [esp+4h] [ebp-4h] BYREF
...
{
    uint classnameIndex; // [esp+0h] [ebp-4h]
...
void __cdecl CMod_LoadBrushRelated(uint version, bool usePvs)
{
...
void CMod_LoadSubmodelBrushNodes()
{
    ...
    uint bmodelIndex; // [esp+14h] [ebp-10h]
    uint16_t *indexes; // [esp+18h] [ebp-Ch]
    uint count; // [esp+1Ch] [ebp-8h] BYREF
    ...
    const float *maxs)
{
    ...
    if (axis >= 0)
    {
        ...
        leafBrushesCopy = (ushort*)CM_Hunk_AllocateTempMemoryHigh(2 * numLeafBrushes, "CMod_PartionLeafBrushes_r");
...
void CMod_LoadBrushes()
{
    ...
    uint allocSizeSides; // [esp+1Ch] [ebp-4Ch]
    uint brushCount; // [esp+20h] [ebp-48h] BYREF
    ...
    uint allocSizeEdges; // [esp+34h] [ebp-34h]
    uint8_t *outEdges; // [esp+38h] [ebp-30h]
    uint materialNum; // [esp+3Ch] [ebp-2Ch]
    uint allocSizeBrushes; // [esp+40h] [ebp-28h]
    int edgeOffset; // [esp+44h] [ebp-24h]
    uint axisIter; // [esp+48h] [ebp-20h]
    int index; // [esp+4Ch] [ebp-1Ch]
    int num; // [esp+50h] [ebp-18h]
    const uint8_t *inEdges; // [esp+54h] [ebp-14h]
    uint sideEdgeCountsCount; // [esp+58h] [ebp-10h] BYREF
    uint sideIter; // [esp+5Ch] [ebp-Ch]
    uint sidesCount; // [esp+60h] [ebp-8h] BYREF
    uint brushIter; // [esp+64h] [ebp-4h]
    ...
    inEdgeCounts = (const byte*)Com_GetBspLump(LUMP_BRUSHSIDEEDGECOUNTS, 1u, &sideEdgeCountsCount);
    ...
    inEdges = (const byte*)Com_GetBspLump(LUMP_BRUSHEDGES, 1u, &edgesCount);
...
void CMod_LoadLeafBrushNodes()
{
    ...
    uint numLeafBrushes; // [esp+8h] [ebp-1Ch]
    uint leafIter; // [esp+Ch] [ebp-18h]
    const DiskLeaf *in; // [esp+10h] [ebp-14h]
    int indexFirstLeafBrush; // [esp+14h] [ebp-10h]
    uint count; // [esp+18h] [ebp-Ch] BYREF
    uint brushIter; // [esp+1Ch] [ebp-8h]
...
void CMod_LoadLeafBrushes()
{
    ...
    uint brushIndex; // [esp+4h] [ebp-14h]
    char *in; // [esp+Ch] [ebp-Ch]
    uint iter; // [esp+10h] [ebp-8h]
    uint count; // [esp+14h] [ebp-4h] BYREF

    in = Com_GetBspLump(LUMP_LEAFBRUSHES, 4u, &count);
    cm.leafbrushes = (ushort*)CM_Hunk_Alloc(2 * count + 2, "CMod_LoadLeafBrushes", 26);
