...
//  uint *huffBytesSeen      82f878d0     msg_mp.obj
//  struct huffman_t msgHuff   82f87cd8     msg_mp.obj

int msgInit;
uint huffBytesSeen[256];
...
{
    uint bitChanged; // [esp+0h] [ebp-10h]
    int j; // [esp+4h] [ebp-Ch]
...
int __cdecl MSG_ReadDeltaEntityStruct(msg_t *msg, int time, const char *from, char *to, uint number)
{
    ...
    uint lc; // [esp+44h] [ebp-14h]
    const NetField *stateFields; // [esp+48h] [ebp-10h]
    uint i; // [esp+54h] [ebp-4h]
    uint ia; // [esp+54h] [ebp-4h]
    ...
    else if (MSG_ReadBit(msg))
    {
        ...
        {
            print = 1;
            Com_Printf(CON_CHANNEL_SYSTEM, "%3i: #%-3i ", msg->readcount, *(uint *)to);
        }
        else
        {
            print = 0;
        ...
        *(uint *)to = number;
        ...
        if (lc <= stateFieldList->count)
        {
            ...
            {
                EntityTypeName = BG_GetEntityTypeName(*((uint *)to + 1));
            ...
            {
                *(uint *)&to[fielda->offset] = *(uint *)&from[fielda->offset];
                ++ia;
                ++fielda;
            }
            return 0;
...
int __cdecl MSG_ReadDeltaClient(msg_t *msg, int time, const clientState_s *from, clientState_s *to, uint number)
{
...
void __cdecl MSG_DumpNetFieldChanges_f()
{
    ...
    uint iArrayNum; // [esp+3Ch] [ebp-1Ch]
