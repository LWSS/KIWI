...
static uint32_t g_debugReadBytesRemote;
static uint32_t g_debugReadBytesSent;
static uint32_t g_debugWriteBytes;
static uint32_t g_debugAck;
static int g_debugAckBytes;

SOCKET ip_debugSocket[2] = { INVALID_SOCKET, INVALID_SOCKET };
SOCKET ip_debugServerSocket[2] = { INVALID_SOCKET, INVALID_SOCKET };

static uint32_t g_debugReadBytes;
...
void __cdecl NET_ShutdownDebug()
{
    ...
    for (i = 0; i < 2; ++i)
    {
        if (ip_debugSocket[i] != INVALID_SOCKET)
        {
            ...
            ip_debugSocket[i] = INVALID_SOCKET;
        }
        if (ip_debugServerSocket[i] != INVALID_SOCKET)
        {
            closesocket(ip_debugServerSocket[i]);
            ip_debugServerSocket[i] = INVALID_SOCKET;
        }
    }
...
static bool Sys_DebugSendAll(SOCKET socket, const char *buffer, int len)
{
    while (len > 0)
    {
        ...
        if (sent <= 0)
        {
            Sys_DebugSocketError("Debug socket send failed");
            return false;
        }
        buffer += sent;
        len -= sent;
...
void Sys_SendDebugReadBytesInternal()
{
    ...
    if (g_debugReadBytes != g_debugReadBytesSent)
    {
        ...
        if (ip_debugSocket[1] == INVALID_SOCKET)
        {
            MyAssertHandler(".\\win32\\win_net.cpp", 1728, 0, "%s", "ip_debugSocket[DEBUG_SOCKET_MSG_REPLY_CHANNEL]");
        }
        if (!Sys_DebugSendAll(ip_debugSocket[1], (const char *)&g_debugReadBytes, sizeof(uint32_t)))
        {
            v0 = NET_ErrorString();
...
char *__cdecl Sys_ReadDebugSocketString()
{
    ...
    if (!Sys_ReadDebugSocketData((char *)&buffer, sizeof(uint32_t), 1))
    {
        Com_Error(ERR_DROP, "Sys_ReadDebugSocketString: connection closed");
    }
    if (buffer >= 0x1000)
    {
        Com_Error(ERR_DROP, "Sys_ReadDebugSocketString: invalid string length %u", buffer);
    }
    if (!Sys_ReadDebugSocketData(in, buffer, 1))
    {
        Com_Error(ERR_DROP, "Sys_ReadDebugSocketString: truncated string");
    }
    in[buffer] = 0;
...
int __cdecl Sys_ReadDebugSocketData(char *buffer, int len, int blocking)
{
    ...
    while (1)
    {
        while (1)
        {
            ...
            iassert(ip_debugSocket[0] != INVALID_SOCKET);
...
void __cdecl Sys_DebugSend(int channel, const char *buf, int len, const char *name)
{
    ...
    uint32_t debugReadBytesRemote; // [esp+4h] [ebp-8h] BYREF
    int read;                      // [esp+8h] [ebp-4h]

    if (ip_debugSocket[channel] != INVALID_SOCKET)
    {
        ...
        while (!Sys_DebugCanSend())
        {
            while (1)
            {
                ...
                read = recv(ip_debugSocket[1], (char *)&g_debugAck + g_debugAckBytes, sizeof(uint32_t) - g_debugAckBytes, 0);
                if (read == -1)
                {
                    break;
                }
                ...
                g_debugAckBytes += read;
                if (g_debugAckBytes == sizeof(uint32_t))
                {
                    g_debugReadBytesRemote = g_debugAck;
                    g_debugAckBytes = 0;
                }
            }
            err = WSAGetLastError();
        ...
        iassert(ip_debugSocket[channel] != INVALID_SOCKET);
        Sys_DebugSendAll(ip_debugSocket[channel], buf, len);
    }
    else
    {
        Sys_DebugSocketError("Sys_DebugSend: Socket closed");
