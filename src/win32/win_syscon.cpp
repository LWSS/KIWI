...
LRESULT __stdcall ConWndProc(HWND__ *hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case 5u:
        SetWindowPos(s_wcd.hwndBuffer, 0, 5, 70, LOWORD(lParam) - 15, HIWORD(lParam) - 100, 0);
        SetWindowPos(s_wcd.hwndInputLine, 0, 5, HIWORD(lParam) - 100 + 78, LOWORD(lParam) - 15, 20, 0);
        s_wcd.windowWidth = LOWORD(lParam);
...
LRESULT __stdcall InputLineWndProc(HWND__ *hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (uMsg == 258 && wParam == 13)
    {
        ...
        if (used + 2 < sizeof(s_wcd.consoleText))
        {
            strncat(s_wcd.consoleText, inputBuffer, sizeof(s_wcd.consoleText) - used - 2);
        }
        if (strlen(s_wcd.consoleText) + 1 < sizeof(s_wcd.consoleText))
        {
...
void __cdecl Sys_CreateConsole(HMODULE hInstance)
{
    ...
    s_wcd.SysInputLineWndProc = reinterpret_cast<WNDPROC>(
        SetWindowLongPtrA(s_wcd.hwndInputLine, GWLP_WNDPROC, (LONG_PTR)InputLineWndProc));
