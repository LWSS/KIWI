...
#define VERT_HASH_PRIME_X            25037
#define VERT_HASH_PRIME_Y            25031
#define VERT_HASH_PRIME_Z            15473
...
#define DEFAULT_BLOCK_SIZE           0.0f
...
#define MAX_MAP_BRUSHEDGES        2097152
...
#define MAX_MAP_COLLISION_VERTS   0x10000
#define MAX_MAP_COLLISION_EDGES   0x60000
#define MAX_MAP_COLLISION_TRIS    0x20000
...
#define MAX_MAP_MODELS            4095
#define MAX_MAP_VISIBILITY        0x200000
/* CoD4 0x40CFB0 bounds byte_10AB7410 against byte_11AB7410. */
...
#define TOOL_PHYSICS               0x0400     /* physics-only brush route (0x41A6C0) */
...
#define NUM_NORMAL_HANDLES       48
#define FIRST_STREAM_HANDLE      49
#define NUM_STREAM_HANDLES       13
#define FIRST_DATABASE_HANDLE    62
#define FIRST_BACKEND_HANDLE     63
...
typedef struct DiskGfxReflectionProbe_s {
    ...
    unsigned char pixels[131064];
} DiskGfxReflectionProbe_t;

typedef char static_assert_disk_reflection_probe_size[
    sizeof(DiskGfxReflectionProbe_t) == 0x20044 ? 1 : -1
];

extern DiskGfxReflectionProbe_t bspReflectionProbes[255];
extern int numBspReflectionProbes;
...
typedef struct BspLeaf_disk_s {
    ...
    int firstCollisionAABB; /* [4]  first collision AABB index */
    int numCollisionAABBs;  /* [8]  collision AABB count */
    int firstLeafBrush;     /* [12] first brush in this leaf */
    int numLeafBrushes;     /* [16] brush count */
    int cellnum;            /* [20] cell number */
} BspLeaf_disk_t;

typedef char static_assert_bsp_leaf_size[sizeof(BspLeaf_disk_t) == 0x18 ? 1 : -1];

/* BspModel_t */
typedef struct BspModel_s {
    float          mins[3];               /* [0]  bounding box minimum */
    float          maxs[3];               /* [12] bounding box maximum */
    unsigned short firstTriSoup;           /* [24] first layered triSoup */
    unsigned short firstTriSoupUnlayered;  /* [26] first unlayered triSoup */
    unsigned short numTriSoups;            /* [28] layered triSoup count */
    unsigned short numTriSoupsUnlayered;   /* [30] unlayered triSoup count */
...
typedef char cod4map_bspmodel_size_must_be_0x30[sizeof(BspModel_t) == 0x30 ? 1 : -1];
...
typedef struct BspTriSoup_s {
    ...
    unsigned char  lightmapIndex;        /* [2] */
    unsigned char  reflectionProbeIndex; /* [3] */
    unsigned char  primaryLightIndex;    /* [4] */
    unsigned char  castsSunShadow;       /* [5] */
    unsigned char  unused[2];            /* [6] */
    int            vertexLayerData;      /* [8] */
    int            firstVertex;          /* [12] */
    unsigned short vertexCount;          /* [16] */
    unsigned short indexCount;           /* [18] */
    int            firstIndex;           /* [20] */
} BspTriSoup_t;

typedef char static_assert_bsp_tri_soup_size[sizeof(BspTriSoup_t) == 0x18 ? 1 : -1];

/* CoD4 0x44A550 qsorts these transient 16-byte records, rather than the
   serialized/expanded triangle records themselves.  Keeping the native
   carrier avoids making its ordering depend on KIWI's larger TriRecord_t. */
typedef struct TrisReorderEntry_s {
    BspTriSoup_t *triSurf;  /* [0] */
    void         *context;  /* [4] native per-vertex-layer context */
    int           minVert;  /* [8] */
    int           maxVert;  /* [12] */
} TrisReorderEntry_t;
...
/* Native transient sun-shadow classifications carried by a TriSoup before
   ...
   CastsSunShadowConfirmed(), not this raw enum value. */
typedef enum CastsSunShadow_e {
    CASTS_SUN_SHADOW_DENIED = 1,
    CASTS_SUN_SHADOW_CONFIRMED = 3
} CastsSunShadow_t;
...
/* BspCollisionVert_t -- CoD4 stores positions directly, without CoD2's
...
/* BspCollisionTri_t -- compact CoD4 collision triangle: three 16-bit
   indices.  Plane/barycentric data remains transient compiler state. */
typedef struct BspCollisionTri_s {
    uint16_t vertIdx[3];  /* [0] collision-vertex indices */
} BspCollisionTri_t;
...
typedef struct BspCell_s {
    ...
    float maxs[3];        /* [12] bounding box maxs */
    union {
        unsigned short aabbTreeIndexPair[2];
        int            aabbTreeIndex; /* compiler-side packed access */
    };                    /* [24] */
    ...
    unsigned char reflectionProbeCount; /* [44] native compiler cell probe list */
    unsigned char reflectionProbes[64]; /* [45] */
    unsigned char padding[3];           /* [109] */
} BspCell_t;

typedef char static_assert_bsp_cell_size[sizeof(BspCell_t) == 0x70 ? 1 : -1];
...
typedef struct DiskPrimaryLight_s {
    unsigned char type;
...
typedef char cod4map_primary_light_size_must_be_0x80[
    sizeof(DiskPrimaryLight_t) == 0x80 ? 1 : -1
];
...
typedef struct ShaderInfo_s {
    ...
    int            gameFlags;      /* [76]  raw MaterialInfo_t::gameFlags (reflection selection) */
    ...
    unsigned char  colorTint[4];   /* material `colorTint`, native BGRA bytes */

    /* COD4's raw material object stays allocated and is referenced by a
       separate 12-byte LoadedMaterial record.  KIWI keeps the compact
       ShaderInfo adapter instead, so retain the native sort keys here and
       sort pointers to these stable cache entries after map parsing. */
    unsigned int   nativeStateBits;
    char           referenceImageName[64];
    char           techniqueSetName[64];
    char           techniquePixelShader[64];
    char           techniqueVertexShader[64];
    unsigned char  techniqueBuildShadowmapDepth;
    unsigned char  techniqueHasLit;
...
typedef struct MatFileHeader_s {
    ...
    unsigned char  gameFlags;     /* [8]  game flags */
    unsigned char  sortKey;       /* [9] */
    unsigned char  atlasRows;     /* [10] */
    unsigned char  atlasColumns;  /* [11] */
    float          maxDeformMove; /* [12] */
    unsigned char  deformFlags;   /* [16] */
    unsigned char  usage;         /* [17] */
    unsigned short toolFlags;     /* [18] tool flags word */
    unsigned int   locale;        /* [20] locale flags */
    unsigned short autoTexScaleW; /* [24] auto texture scale width */
    unsigned short autoTexScaleH; /* [26] auto texture scale height */
    float          tessSize;      /* [28] tessellation size (0 = use default) */
    int            surfaceFlags;  /* [32] surface flags */
    int            contentFlags;  /* [36] content flags */
} MatFileHeader_t;

typedef char static_assert_mat_file_header_size[sizeof(MatFileHeader_t) == 0x28 ? 1 : -1];
...
typedef struct WindingList_s {
    Winding_t *winding;
    struct WindingList_s *next;
} WindingList_t;

/* Native common/primarylights_region.cpp transient carriers. */
typedef struct PrimaryLightRegionDesc_s {
    int   type;             /* +0x00 */
    float origin[3];        /* +0x04 */
    float dir[3];           /* +0x10 */
    float radius;           /* +0x1C */
    float cosHalfFov;       /* +0x20 */
} PrimaryLightRegionDesc_t;

typedef struct PrimaryLightRegionNode_s {
    float plane[4];                         /* +0x00 */
    float dir[3];                           /* +0x10 */
    float cosHalfFov;                       /* +0x1C */
    int exterior;                           /* +0x20 */
    Winding_t *w;                           /* +0x24 */
    void *sourceSurface;                    /* +0x28 */
    struct PrimaryLightRegionNode_s *next;  /* +0x2C */
} PrimaryLightRegionNode_t;

typedef char cod4map_primary_light_region_desc_size_must_be_0x24[
...
/*
 * Merge index winding.  The native COD4 payload is a count followed directly
 * by the index array.  auxData is a KIWI compatibility sidecar for the
 * shadow-caster merge route only; the normal COD4 concave merge route leaves
 * it null and never depends on it.
 */
typedef struct IntWinding_s {
    int   numpoints; /* [0] vertex count */
    void *auxData;   /* KIWI shadow-route compatibility sidecar */
    int   indices[]; /* native payload: vertex indices */
} IntWinding_t;
...
typedef struct BrushSide_s {
    ...
    BrushAdjacencyWinding_t *adjacencyWinding; /* [76] collision-side indices */
    ShaderInfo_t   *shaderInfo;       /* [80]  primary material/shader */
    union { ShaderInfo_t *lmShaderInfo; const char *materialName; }; /* [84] */
    int             contentFlags;     /* [88]  content flags */
    union {
        int         surfaceFlags;     /* [92]  surface flags */
        int         smoothAngleAsInt; /* [92]  compatibility alias */
    };
    int             toolFlags;        /* [96]  tool flags */
    int             compileFlags;     /* [100] portal state */
    unsigned char   bevel;             /* [104] bevel flag */
    unsigned char   visible;           /* [105] transient visibility state */
    unsigned char   sidePad[2];
    int             value;             /* [108] portal state */
...
typedef struct Brush_s {
    ...
    int             mapInfoIndex;        /* [12] source map/prefab index */
    union { ShaderInfo_t *contentShaderInfo; const char *contentShader; }; /* [16] */
    int             contentFlags;      /* [20]  content/surface flags (for EmitBrushes) */ 
    unsigned char detail;              /* [24] detail rather than structural */
    unsigned char forceVisible;        /* [25] */
    union { unsigned char isOpaque; unsigned char hasPortals; }; /* [26] */
    unsigned char isOccluder;          /* [27] */
    int outputNum; int bspBrushIdx; int parentBrush; int siblingBrush;
...
typedef struct Entity_s {
    ...
    Brush_t       *brushes;            /* [12] ordinary brush list */
    Brush_t       *physicsBrushes;     /* [16] CoD4 physics-brush list */
    Patch_t       *patches;            /* [20] patch/mesh list */
    TerrainNode_t *terrainList;        /* [24] legacy terrain/extra geometry list */
    int            firstDrawSurf;      /* [28] first intermediate draw surface */
    int            firstTriSoup;       /* [32] first layered triangle surface */
    int            firstTriSoupSimple; /* [36] first unlayered triangle surface */
    Epair_t       *epairs;             /* [40] key-value pair list */
    int            mapInfoIndex;       /* [44] source map/prefab information index */
    int            entityNum;          /* [48] original source entity number */
} Entity_t;
...
typedef struct Patch_s {
    ...
    int                smoothing;    /* [44] smoothing_hard/smooth/smooth_other */
    int                mapInfoIndex; /* [48] source map/prefab information index */
    int                entityNum;    /* [52] source entity number */
    const char        *mapName;      /* [56] map name string pointer */
    int                outputNum;    /* [60] output surface number (init -1) */
    int                terrainSortKey; /* [64] terrain grouping key */
...
typedef struct Node_s {
    ...
    int                     opaque;            /* [36]  solid state: 0=open, 1=solid, 2=mixed internal node */
    void                   *side;              /* [40]  side that created this node */
    struct Node_s          *children[2];       /* [44]  child nodes (front/back) */
    int                     tinyportals;       /* [52]  tiny portal count */ 
    float                   referencepoint[3]; /* [56]  reference point */ 
    int                     portalState;       /* [68]  native portal/BSP scratch state */
    ...
    struct DrawSurfRef_s   *drawSurfRefs;      /* [84]  draw surface reference list */ 
    void                   *collisionAABBs;    /* [88]  collision AABB tree */
    int                     occupied;          /* [92]  entity reach distance */ 
    Entity_t               *occupant;          /* [96]  entity that reached leaf */
    int                     cellnum;           /* [100] cell number */ 
    struct Portal_s        *portals;           /* [104] linked list of portals */
} Node_t;
...
typedef struct Tree_s {
    ...
    Node_t  outside_node; /* [4]   embedded outside node (108 bytes) */
    float   mins[3];      /* [112] tree bounding box min */
...
typedef struct DrawVert_s {
    ...
    int   smoothing;       /* native transient vertex +100: hard=0, smooth=1, smooth_other=2 */
} DrawVert_t;
...
/* Native cod4map.exe MapDrawSurf (0x44 bytes).  KIWI's DrawSurf_t grew to
   support its donor-era pipeline, so this remains a parallel carrier rather
   than constraining the expanded surface used by existing consumers. */
typedef union MapDrawSurfUnion_u {
    struct {
        BrushSide_t *side;
        int          unused[2];
    } brush;
...
/* Native +32/+36 have type-dependent meanings.  Brush sides retain their
   brush/map-info identity there; terrain and patches retain map-info/name. */
typedef union MapDrawSurfSourceIndex_u {
    int brushNum;
    int mapInfoIndex;
} MapDrawSurfSourceIndex_t;

typedef union MapDrawSurfSourceInfo_u {
    int         mapInfoIndex;
    const char *mapName;
} MapDrawSurfSourceInfo_t;

typedef struct MapDrawSurf_s {
    ShaderInfo_t         *material;          /* [0]  native material carrier */
    void                 *lightmapMaterial;  /* [4]  native lightmap-material carrier */
    int                   reflectionProbeIndex; /* [8]  allocator initializes 255 */
    int                   lightmapIndex;        /* [12] allocator initializes 256 */
    int                   contentFlags;         /* [16] source-dependent material field */
    int                   toolFlags;            /* [20] source-dependent material field */
    int                   compileOrSmoothing;   /* [24] source-dependent material field */
    int                   entityNum;         /* [28] */
    MapDrawSurfSourceIndex_t sourceIndex;    /* [32] brushNum or mapInfoIndex */
    MapDrawSurfSourceInfo_t  sourceInfo;     /* [36] mapInfoIndex or mapName */
    int                   vertCount;         /* [40] */
...
typedef struct TerrainNode_s {
    ...
    int                   smoothing;    /* [20] smoothing mode inherited from Patch_t */
    int                   outputNum;    /* [24] output surface number */
    int                   numIndexes;   /* [28] total index count (3 * triCount) */
    unsigned short       *indexes;      /* [32] index array (unsigned shorts) */
    int                   numVerts;     /* [36] vertex count */
...
typedef struct TriSurfProps_s {
    ShaderInfo_t          *si;              /* [0x00] pointer to ShaderInfo_t cache entry */
    int                    lmapGroupId;     /* [0x04] native creator uses -1; 0x454F90 writes group id */
    int                    lmapIndex;       /* [0x08] native creator uses 31/LIGHTMAP_NONE; 0x454F90 writes atlas index */
    ...
    struct TriSurfProps_s *freeListNext;    /* [0xA8] free-list/hash next pointer */
} TriSurfProps_t;
...
typedef struct TriSurfPropsSidecar_s {
    ...
    MapDrawSurf_t                     *mapDrawSurf; /* native props->ds carrier */
    float                              smoothAngle;
    int                                smoothing;
    int                                lightStyle;
    unsigned char                      reflectionProbeIndex;
    unsigned char                      reflectionCellMode;
    struct TriSurfPropsSidecar_s      *next;
} TriSurfPropsSidecar_t;

/* TriSurf_t.  The first 0x40 bytes are the native CoD4 carrier used by
   tris.cpp 0x43C560..0x43D260.  Fields after that prefix are KIWI's donor
   compatibility payload and must not displace a native field. */
typedef struct TriSurf_s {
    Winding_t        *winding;     /* [0]  native w */
    void             *auxData;     /* [4]  native fw; KIWI aux-data compatibility */
    TriSurfProps_t   *props;       /* [8]  native props */
    float             mins[3];     /* [12] native bounds mins */
    float             maxs[3];     /* [24] native bounds maxs */
    Winding_t        *origWinding; /* [36] native transient wOrig */
    struct TriSurf_s *prev;        /* [40] native list prev */
    struct TriSurf_s *next;        /* [44] native list next */
    struct TriSurf_s *gridPrev;    /* [48] native grid-tree prev */
    struct TriSurf_s *gridNext;    /* [52] native grid-tree next */
    int               holeCount;   /* [56] native hole count */
    WindingAuxPair_t *holes;       /* [60] native holes */
    int               auxElemSize; /* [64] KIWI aux-data element size */
    struct TriSurf_s *groupNext;   /* [68] KIWI group link */
    struct TrisLmapTransient_s *lmap; /* [72] KIWI lightmap transient */
} TriSurf_t;
...
/* Native tris_lightmap.cpp transient carriers.  They remain separate from
   ...
   draw-surface lightmap path. */
typedef struct TrisLmapGroup_s TrisLmapGroup_t;

typedef struct TrisLmapTransient_s {
    float                   bounds[2][2];    /* [0]  UV mins/maxs */
    float                   vecs[2][4];      /* [16] lightmap S/T vectors */
    TrisLmapGroup_t        *group;           /* [48] owning group */
    struct TriSurf_s       *prevInGroup;     /* [52] */
    struct TriSurf_s       *nextInGroup;     /* [56] */
    struct TriSurf_s       *nextInMaterial;  /* [60] */
} TrisLmapTransient_t;

typedef struct TrisLmapGroup_s {
    float                   bounds[2][2];    /* [0]  UV mins/maxs */
    int                     id;              /* [16] stable group identifier */
    unsigned int           *usedGroupBits;   /* [20] compatibility bitset */
    TriSurf_t              *firstSurf;       /* [24] */
    struct TrisLmapGroup_s *prev;            /* [28] */
...
typedef struct TrisLmapAssignmentPayload_s {
    ...
    int   lightmapIndex;
    float vecs[2][4];
} TrisLmapAssignmentPayload_t;

typedef struct TrisLmapAssignmentSidecar_s {
    TriSurf_t                         *surf;
    TriSurfProps_t                    *props;
    TrisLmapAssignmentPayload_t        payload;
    struct TrisLmapAssignmentSidecar_s *next;
} TrisLmapAssignmentSidecar_t;

/* Native 0x454820 uses a 512*512 table of these entries (exactly 1 MiB). */
typedef struct TrisLmapSpan_s {
    unsigned short width;
    unsigned short height;
} TrisLmapSpan_t;

typedef struct TrisLmapRasterScratch_s {
    TrisLmapSpan_t *span;
    int              originX;
    int              originY;
...
typedef struct DrawSurf_s {
    ...
    /* Native DrawSurf keeps the source map-info carrier alongside entity and
       map name.  The expanded KIWI transient keeps it out of its recovered
       0x44-byte prefix, but terrain/patch construction must preserve it. */
    int                mapInfoIndex;
} DrawSurf_t;
...
typedef struct TriRecord_s {
    ...
    unsigned char reflectionProbeIndex; /* [160] inherited from TriSurfProps_t */
...
typedef struct GridTreeNode_s {
    float      min;          /* [0] native lower traversal bound */
    float      max;          /* [4] native upper traversal bound */
...
/* Callback used by the native grid-tree segment traversal.  Return nonzero
   to continue the trace, or zero to terminate it. */
typedef unsigned char (*GridTreeSegmentCallback_t)(TriSurf_t *surf,
                                                    float *segmentStart,
                                                    float *segmentEnd,
                                                    int userData);
...
typedef struct CoalesceBspNode_s {
    ...
    union {                  
        float                 splitDist; /* compatibility float view */
        int                   splitCoord;/* [4]  native fixed-point split */
...
typedef struct CollisionEdge_s {
    ...
    int                     walkable;        /* [4]  first-triangle normal Z >= 0.7 */
    int                     reverseMatch;    /* [8]  reverse/convex match flag */
    int                     processed;       /* [12] has been processed (0/1) */
    int                     matId;           /* [16] material ID */
    float                   normalZ;         /* [20] edge normal Z component */
    int                     vertA;           /* [24] vertex index A */
    int                     vertB;           /* [28] vertex index B */
    int                     oppositeVert;    /* [32] opposite vertex index */
    struct CollisionEdge_s *hashNext;        /* [36] hash chain next pointer */
    /* Retained only for the existing border helper; CoD4 reconstructs these
       values from the compact fields above instead of serializing an edge. */
    float                   vertPoolData[3];
    float                   perpVec[3];
    float                   crossVec[3];
    float                   edgeDir[3];
    float                   edgeLen;
...
typedef struct CollisionTri_s {
    ...
    int         edgeIdx[3];     /* [80] edge indices (from CollisionAddEdge) */
    int         duplicateKey;   /* [92] order-independent vertex key */
    uint16_t    vertIdx[3];     /* [96] vertex indices into g_cmVertPool */
    uint8_t     alpha;          /* [102] collision triangle opacity */
    uint8_t     reserved103;
} CollisionTri_t;

/* CmVert_t */
typedef struct CmVert_s {
    int   partitionId; /* [0]  partition ID for grouping connected tris */
    float xyz[3];      /* [4]  vertex position */
} CmVert_t;
...
/* Current per-material preferred-atlas list (native 0x11BA8424). */
extern LmAllowedNode_t *lmAllowedList;
...
typedef struct Iwd_s {
    ...
    int           hasOpenFile;      /* [772] shared unzip handle ownership */
    int           numFiles;         /* [776] number of files in archive */
    char          referenced;       /* [780] referenced flag */
    int           hashSize;         /* [784] hash table size */
...
typedef struct Searchpath_s {
    ...
    int                  ignore;    /* [16] compiler search-path control */
...
/* Native 32-bit DvarValue is a 16-byte by-value carrier.  Vectors are
 * inline; they are not separately allocated buffers. */
typedef union DvarValue_u {
    int            enabled;  /* [0] bool value */
    int            integer;  /* [0] int / enum value */
    float          value;    /* [0] float value */
    float          vector[4];/* [0] vec2/vec3/vec4 value */
...
/* Dvar_t matches the native 32-bit layout through hashNext.  `next` is a
 * map-tool list link kept after the native object for the existing sorted
...
struct Dvar_s {
    ...
    unsigned char   modified;    /* [11] */
    DvarValue_t     current;     /* [12] */
    DvarValue_t     latched;     /* [28] */
    DvarValue_t     reset;       /* [44] */
    DvarLimits_t    domain;      /* [60] */
    bool (__cdecl  *domainFunc)(Dvar_t *, DvarValue_t); /* [68] */
    Dvar_t         *hashNext;   /* [72] */
    Dvar_t         *next;       /* map-tool sorted-list link */
};
...
#define g_parseDepth         (g_parse.parseInfoNum)
...
int BrushToPoints(Brush_t *brush, BrushSide_t *sides, int numSides, float capsuleRadius, float capsuleHalfHeight,
                  int checkBevel,
...
void ValidateBspTriangleData(const BspTriSoup_t *triSoups, unsigned int triSoupCount, unsigned int diskVertCount, const unsigned short *diskIndices, unsigned int diskIndexCount);
...
Entity_t *FindEntityByKeyValue(Entity_t *entities, int entityCount, const char *key, const char *value);
...
int ClipWinding(Winding_t *inWinding, float *planeNormal, float planeDist, float epsilon,
                Winding_t **outFront, Winding_t **outBack);
void ValidateWinding(Winding_t *w);

/* Common/primarylights_region.cpp */
...
int WindingsAreCoplanar(const Winding_t *windingA, const float *planeA, const Winding_t *windingB, const float *planeB);
...
/* Common/poly2d.cpp */
typedef void (__cdecl *Poly2DSubdivideCallback)(float logArea, float *centroid, float *coords, int vertCount, void *userData, int polygonIndex);
void SplitPoly2D(const float *coords, int vertCount, unsigned int axis, float splitCoord, float *coordsFront, int *frontCountOut, float *coordsBack, int *backCountOut);
int Subdivide2DPolygonGrid(float *polyBuffers, int vertCount, int gridCountX, int gridCountY, float gridMinsX, float gridMinsY, float gridSizeX, float gridSizeY, Poly2DSubdivideCallback callback, void *userData);
...
int ClassifyWindingAgainstPlane(Face_t *winding, float *planeNormal, float planeDist, int axis);
int CheckPlaneAgainstAllWindings(float *planeNormal, Face_t *windingList, int axis);
void SelectSplitPlane_DirectEval(int *evalResult, Face_t *windingList, int axis);
void SelectSplitPlane_EvaluateAxes(int *evalResult, Face_t *windingList, signed int faceCount, int axis);
...
void FreeLMBlock(int lightmapIdx, int y, int x, int height, int width);
...
DrawSurf_t *DrawSurfaceForTerrain(TerrainNode_t *terrain, Patch_t *sourcePatch);
...
Node_t *FillOutside_r(Node_t *node);
...
/* Shadows.c */
int CastsSunShadowConfirmed(unsigned char castsSunShadow);
int ConfirmCoalescedSunShadowCategory(TriSurf_t *surf);
char SetCoalescedSunShadowCategory(TriSurf_t *surf, char category);
int PreprocessSunShadowTriCategories(void);
int DisableSunShadowTrisForEntityRange(void);
void ClassifySunShadowTri(MapDrawSurf_t *drawSurf, float *sunDirection);
int SunShadowVertexIsExposed(MeshVert_t *vert);
unsigned char SunShadowSegmentHitCallback(TriSurf_t *surf, float *segmentStart,
                                          float *segmentEnd, int userData);
...
void CullBrushSideVisibleHulls(Brush_t *brushes, Node_t *treeRoot);
...
void TrisPropsSidecar_Create(TriSurfProps_t *props, float smoothAngle, int smoothing,
...
void TrisLmap_SaveAllocatorCheckpoint(void);
void TrisLmap_RestoreAllocatorCheckpoint(void);
void TrisLmap_ClearAllocatorCheckpoint(void);
...
int TrisLmap_AssignmentNeedsClone(const TriSurf_t *surf, const TriSurfProps_t *props, const TrisLmapAssignmentPayload_t *payload);
void TrisLmap_SetAssignment(TriSurf_t *surf, TriSurfProps_t *props, const TrisLmapAssignmentPayload_t *payload);
void TrisLmap_CopyAssignmentForPropsClone(TriSurf_t *sourceSurf, TriSurfProps_t *sourceProps, TriSurf_t *cloneSurf, TriSurfProps_t *cloneProps);
...
int Tris_ReorderSortCompare(const void *entry0, const void *entry1);
int Tris_FindNextEntity(int entityIndex);
void Tris_ReorderDrawVerts();
void Tris_ReorderUnlayeredDrawVerts();
...
Winding_t **ClipTriWinding(Winding_t *in, Winding_t *reference, float *planeNormal, float planeDist, float epsilon, Winding_t **front, Winding_t **back);
WindingAuxPair_t *ClipTriWindingWithAux(WindingAuxPair_t *in, Winding_t *reference, int auxElemSize, float *planeNormal, float planeDist, float epsilon, WindingAuxPair_t *front, WindingAuxPair_t *back);
...
Winding_t *MergeWindingsAtBridge(WindingAuxPair_t *vertsInner, WindingAuxPair_t *vertsOuter,
                                 int bridgeInner, int bridgeOuter, int auxElemSize,
                                 WindingAuxPair_t *verts);
...
int InsertHole(WindingAuxPair_t *verts, float area, float *plane,
               WindingAuxPair_t *holes, float *holeArea, int holeCount,
               int auxElemSize, int a8);
int FindHoles(WindingAuxPair_t *verts, float *plane, WindingAuxPair_t *holes,
              float *holeArea, int auxElemSize, int extraParam);
...
IntWinding_t *BuildIndexList(WindingAuxPair_t *vertsPair, int auxElemSize);
char ValidateHoles(TriSurf_t *ts, float epsilon);
IntWinding_t *RemoveSharedBoundaryPoints(IntWinding_t *w0, int auxElemSize, int startIdx, int dupCount);
...
/* Native 0x45B6F0's mode-4 callback route.  It owns only an index worklist
   and never changes the source TriSurf_t; a local winding is made only when
   repairing a self-intersection. */
typedef void (*TrisLmapTessCallback_t)(Winding_t *w, Winding_t *wOrig, TriSurfProps_t *props,
                                       int vertIdx0, int vertIdx1, int vertIdx2, int visGroupIndex,
                                       void *userData);
int TrisLmap_TesselateWinding(const TriSurf_t *surf, int visGroupIndex,
...
int FS_FOpenFileWrite(char *filename);
...
int FS_HandleForFile(int thread);
...
int FS_HandleOpen(const char *qpath, const char *ospath, int thread);
...
int FS_FOpenFileRead(const char *filename, int *fileHandleOut, int thread);
...
void AxisToQuat(const float *axis, float *out);
float Vec4Dot(const float *a, const float *b);
float Vec4LengthSq(const float *v);
int Vec4IsNormalized(const float *v);
float *Vec3Mul(const float *a, const float *b, float *product);
float *Vec4Lerp(const float *from, const float *to, float frac, float *result);
float Q_fabs(float value);
void FastSinCos(float radians, float *sinOut, float *cosOut);
float *ConvertQuatToMat(float *mat, float *axis);
float *MatrixTransformVectorQuatTrans(float *in, float *mat, float *out);
...
int PointInBounds2D(float *point, float *mins, float *maxs);
int PointInBounds(float *point, float *mins, float *maxs);
int BoundsIntersect(float *mins1, float *maxs1, float *mins2, float *maxs2);
int BoundsIntersect2D(float *mins1, float *maxs1, float *mins2, float *maxs2);
...
void AddBoundsToBounds2D(float *srcMins, float *srcMaxs, float *dstMins, float *dstMaxs);
...
int ProjectedWindingContainsCoplanarPoint(const float *verts, int vertCount, int x, int y, const float *point, float epsilon);
...
int CullBoxFromSphere(const float *sphereOrg, float radius, const float *boxCenter, const float *boxHalfSize);
int CullBoxFromConicSectionOfSphere(const float *coneOrg, const float *coneDir, float cosHalfFov, float radius, const float *boxCenter, const float *boxHalfSize);
...
void Dvar_MakeExplicitType(Dvar_t *dvar, const char *dvarName, unsigned char type, unsigned short flags, DvarValue_t resetValue, DvarLimits_t domain);
...
DvarValue_t *Dvar_ClampValueToDomain(DvarValue_t *result, unsigned char type, DvarValue_t value, DvarValue_t resetValue, DvarLimits_t domain);
...
void Dvar_Reregister(Dvar_t *dvar, const char *dvarName, unsigned char type, unsigned short flags, DvarValue_t resetValue, DvarLimits_t domain, const char *description);
Dvar_t *Dvar_RegisterVariant(char *dvarName, unsigned char type, unsigned short flags, DvarValue_t value, DvarLimits_t domain, const char *description);
Dvar_t *Dvar_RegisterNew(char *dvarName, unsigned char type, unsigned short flags, DvarValue_t value, DvarLimits_t domain, const char *description);
Dvar_t *Dvar_FindOrRegisterVariant(short flags, char *dvarName, int type, char *value, int domainMin, int domainMax);
Dvar_t *Dvar_RegisterBool(const char *dvarName, char value, short flags, const char *description);
...
int COM_MatchToken(char **parsePtr, const char *match, int warnOnly);
...
char *COM_StripExtension(char *input, char *output);
int BigShort(int l);
int BigLong(int l);
int BigFloat(int l);
int ShortSwap(short l);
...
char *Info_RemoveKey(char *s, char *key);
char *Info_RemoveKey_Big(char *s, char *key);
...
float *MatrixTransposeTransformVector43(float *mat, float *in, float *out);
...
extern BspAabbTreeEntry_t       bspUnlayeredAabbTrees[MAX_MAP_AABBTREES];
extern BspBrushSide_t           bspBrushSidesData[MAX_MAP_BRUSHSIDES];    
extern unsigned char            bspBrushSideEdgeCounts[MAX_MAP_BRUSHSIDES];
extern unsigned char            bspBrushEdges[MAX_MAP_BRUSHEDGES];
...
extern unsigned char            bspCollisionEdgeWalkable[((3 * MAX_MAP_COLLISION_TRIS + 7) / 8 + 3) & ~3];
extern BspCullGroup_t           bspCullGroups[MAX_MAP_CULLGROUPS];    
extern BspCullGroup_t           bspUnlayeredCullGroups[MAX_MAP_CULLGROUPS];
extern BspDrawVert_t            bspDrawVerts[MAX_MAP_DRAW_VERTS];    
extern BspDrawVert_t            g_drawVertexBuf[MAX_MAP_DRAW_VERTS + 1];    
extern BspDrawVert_t            bspUnlayeredDrawVerts[MAX_MAP_DRAW_VERTS];
extern BspDrawVert_t            g_unlayeredDrawVertexBuf[MAX_MAP_DRAW_VERTS + 1];
...
/* Native 0x442690 reorders these cell lists during the mode-4 lightmap
   grouping pass; tris.cpp owns the storage. */
extern TriSurf_t                *triSurfCellArray[TRISURFS_CELL_ARRAY_SIZE];
...
extern BspTriSoup_t             bspUnlayeredTriangles[MAX_MAP_TRISOUPS];
...
extern unsigned short           bspUnlayeredDrawIndexes[MAX_MAP_DRAW_INDEXES];
...
extern int numBSPUnlayeredAabbTrees;
extern int numBSPBrushSides;
extern int numBSPBrushEdges;
...
extern int numBSPUnlayeredDrawIndexes;
extern int numBSPUnlayeredDrawVerts;
extern int numBSPUnlayeredDrawVertsEmitted;
...
extern DiskPrimaryLight_t bspPrimaryLights[255];
extern unsigned char bspLightRegions[255];
extern int numBspPrimaryLights;
...
extern int numBSPUnlayeredTriSoups;
...
extern int collisionNodeLimit;
extern float smoothAngle;
extern float sampleScale;
extern float listSlowEntities;
extern float warnLayerArea;
extern int   brushMethod;
extern int   byteswapMode;
extern int   debugPortals;
extern int   debugLightmaps;
extern int   displayCollMapWarnings;
extern int   staticModelCollMaps;
extern int   warnLayerUses;
extern int   reflectionDebug;
extern int   fastvis;
...
extern MapDrawSurf_t      g_nativeMapDrawSurfs[MAX_MAP_DRAW_SURFS];
...
extern float       g_smTransposedInvMatrix[16];
extern float       g_smViewProjMatrix[16];
