...
static FILE *ZipFileOpen(const char *path)
{
	return fopen(path, "rb");
}

static unsigned int ZipFileRead(void *buffer, unsigned int length, FILE *file)
{
	return (unsigned int)fread(buffer, 1, length, file);
}

static long ZipFileSize(FILE *file)
{
...
#define ZIP_fopen ZipFileOpen
#define ZIP_fclose fclose
#define ZIP_fseek fseek
#define ZIP_fread ZipFileRead
#define ZIP_ftell ZipFileSize
...
#ifndef ALLOC
//# define ALLOC(size) (Z_Malloc(size, "zip", qfalse))
#define ALLOC(size) (malloc(size))
#endif
#ifndef TRYFREE
//# define TRYFREE(p) {if (p) Z_Free(p, 'kisk');} // KISAK EPIC CUSTOM (probably breaks something)
# define TRYFREE(p) {if (p) free(p); }// KISAK EPIC CUSTOM (probably breaks something)
#endif
...
static uLong unzlocal_SearchCentralDir(ZIP_FILE *fin)
{
	...
	while (uBackRead<uMaxBack)
	{
		...
		if (ZIP_fread(buf,(uInt)uReadSize,fin)!= uReadSize)
			break;
												  ...
												  uLong commentBufferSize)
{
	...
	if ((err==UNZ_OK) && (szFileName!=NULL))
	{
		...
		if ((file_info.size_filename>0) && (fileNameBufferSize>0))
			if (ZIP_fread(szFileName,(uInt)uSizeRead,s->file)!=uSizeRead)
				err=UNZ_ERRNO;
		lSeek -= uSizeRead;
	}
	...
	if ((err==UNZ_OK) && (extraField!=NULL))
	{
		...
		if (lSeek!=0) {
			if (ZIP_fseek(s->file,lSeek,0)==0)
				lSeek=0;
			else
				err=UNZ_ERRNO;
		}
		if ((file_info.size_file_extra>0) && (extraFieldBufferSize>0)) {
			if (ZIP_fread(extraField,(uInt)uSizeRead,s->file)!=uSizeRead)
				err=UNZ_ERRNO;
		}
	...
	if ((err==UNZ_OK) && (szComment!=NULL))
	{
		...
		if ((file_info.size_file_comment>0) && (commentBufferSize>0)) {
			if (ZIP_fread(szComment,(uInt)uSizeRead,s->file)!= uSizeRead)
				err=UNZ_ERRNO;
		}
...
extern int unzOpenCurrentFile (unzFile file)
{
	...
	if (!Store)
	{
	  ...
	  err = inflateInit2_(&pfile_in_zip_read_info->stream, -15, "1.1.4", sizeof(z_stream));
	  if (err == Z_OK)
