    ...
    float *productsOfInertia)
{
    ...
    uint j; // [esp+80h] [ebp-A0h]
    float pointSum[3]; // [esp+84h] [ebp-9Ch] BYREF
    uint k; // [esp+90h] [ebp-90h]
...
uint __cdecl Xmodel_CountPhysicsCollMapGeoms(const char **file, const char *name)
{
    int CurrentParseLine; // eax
    uint brushCount; // [esp+B4h] [ebp-8h]
...
char __cdecl IsBoxInBrush(const float *boxCenter, const float *boxHalfLengths, const BrushWrapper *brush)
{
    ...
    uint sideIndex; // [esp+2Ch] [ebp-4h]
    uint sideIndexa; // [esp+2Ch] [ebp-4h]
...
bool __cdecl Xmodel_ParsePhysicsBox(const char **file, PhysGeomInfo *geom)
{
    ...
    {
        Com_PrintError(CON_CHANNEL_ANIM, "Expecting '{' while parsing physics box"); // LWSS: add from blops. cod4 says "cylinder" here
        return 0;
    }
}
    ...
    uint brushnum)
{
    ...
    uint sideIndexK; // [esp+20h] [ebp-18h]
    uint sideIndexKa; // [esp+20h] [ebp-18h]
    float negatedNormal[3]; // [esp+24h] [ebp-14h] BYREF
    uint sideIndexJ; // [esp+30h] [ebp-8h]
    uint sideIndexI; // [esp+34h] [ebp-4h]
...
uint __cdecl GetPlaneIntersections(
    const float (*planes)[4],
    uint planeCount,
    ...
    uint maxPtCount)
{
    ...
    uint ptCount; // [esp+14h] [ebp-28h]
    float xyz[3]; // [esp+18h] [ebp-24h] BYREF
    uint sideIndex[3]; // [esp+24h] [ebp-18h]
    uint planeIndex[3]; // [esp+30h] [ebp-Ch] BYREF
...
PhysGeomList *__cdecl XModel_LoadPhysicsCollMap(const char *name, void *(__cdecl *Alloc)(int))
{
    ...
    uint geomCount; // [esp+410h] [ebp-8h]
