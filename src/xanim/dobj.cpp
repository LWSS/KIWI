...
void __cdecl DObjCreate(DObjModel_s *dobjModels, uint numModels, XAnimTree_s *tree, DObj_s *obj, __int16 entnum)
{
...
void __cdecl DObjDumpCreationInfo(DObjModel_s *dobjModels, uint numModels)
{
    const char *Name; // eax
    uint j; // [esp+0h] [ebp-14h]
    int numBones; // [esp+4h] [ebp-10h]
    uint boneIndex; // [esp+8h] [ebp-Ch]
...
void __cdecl DObjTraceline(DObj_s *obj, float *start, float *end, unsigned __int8 *priorityMap, DObjTrace_s *trace)
{
    ...
    uint localBoneIndex; // [esp+344h] [ebp-6Ch]
    ...
    uint lowestPriority; // [esp+364h] [ebp-4Ch]
    ...
    uint currentPriority; // [esp+3ACh] [ebp-4h]
    ...
    iassert(skel);
    iassert(obj->duplicateParts);
    pos = (const byte*)(SL_ConvertToString(obj->duplicateParts) + 16);
    ...
    models = obj->models;
    modelParents =  (const byte*)&models[obj->numModels];
    numModels = obj->numModels;
    j = 0;
LABEL_17:
    if (j < numModels)
...
{
    uint boneIndex; // [esp+0h] [ebp-14h]
    XModel *model; // [esp+4h] [ebp-10h]
    uint numModels; // [esp+8h] [ebp-Ch]
...
int __cdecl DObjGetBoneIndex(const DObj_s *obj, uint name, unsigned __int8 *index)
{
    int j; // [esp+0h] [ebp-18h]
    int ja; // [esp+0h] [ebp-18h]
    uint boneIndex; // [esp+4h] [ebp-14h]
    int numModels; // [esp+8h] [ebp-10h]
    XModel *model; // [esp+Ch] [ebp-Ch]
    XModel *modela; // [esp+Ch] [ebp-Ch]
    uint localBoneIndex; // [esp+10h] [ebp-8h]
...
int __cdecl DObjGetModelBoneIndex(const DObj_s *obj, const char *modelName, uint name, unsigned __int8 *index)
{
    int j; // [esp+0h] [ebp-18h]
    int ja; // [esp+0h] [ebp-18h]
    uint boneIndex; // [esp+4h] [ebp-14h]
...
void __cdecl DObjSetHidePartBits(DObj_s *obj, const uint *partBits)
{
