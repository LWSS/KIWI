...
char __cdecl Com_LoadVolumeFalloffCurve(const char *name, SndCurve *curve)
{
    ...
    byte buffer[8192]; // [esp+58h] [ebp-2010h] BYREF
...
{
    vassert((sourceChannelCount == 1 || sourceChannelCount == 2), "(sourceChannelCount) = %i", sourceChannelCount);
    return &speakerMap->channelMaps[sourceChannelCount == 2][SND_IsMultiChannel()];
}

uint __cdecl Com_HashAliasName(const char *name)
{
    ...
    if (g_sa.initialized[system])
    {
        ...
        for (i = 0; i < g_sa.aliasInfo[system].count; ++i)
        {
            if ((aliases[i].flags & 0xC0) >> 6 == 1)
            {
                ...
                if (aliases[i].soundFile->exists)
                {
                    fileMem = SND_GetSoundFileSize((uint*)&aliases[i].soundFile->u.loadSnd->sound.info.format);
                    totalMem += fileMem;
...
void __cdecl Com_RefreshSpeakerMaps_f()
{
    ...
    iassert(g_sa.speakerMapsInitialized);
