...
iwd_t *__cdecl FS_LoadZipFile(char *zipfile, char *basename)
{
    ...
    iwd->checksum = Com_BlockChecksumKey32((const byte *)fs_headerLongs, 4 * fs_numHeaderLongs, 0);
    if (fs_checksumFeed)
        iwd->pure_checksum = Com_BlockChecksumKey32((const byte*)fs_headerLongs, 4 * fs_numHeaderLongs, fs_checksumFeed);
    else
...
void __cdecl FS_AddIwdFilesForGameDirectory(char *path, char *pszGameFolder)
{
    ...
    ospath[&ospath[strlen(ospath) + 1] - &ospath[1] - 1] = 0; // strip the trailing slash
...
void __cdecl Com_ReadCDKey()
{
    ...
    //uint size; // [esp+0h] [ebp-28h] BYREF
    //uint type; // [esp+4h] [ebp-24h] BYREF
...
void __cdecl FS_Startup(char *gameName)
{
    ...
    FS_Path_f();
    Dvar_ClearModified((dvar_s *)fs_gameDirVar); // We just loaded, it's not modified
    Com_Printf(CON_CHANNEL_FILES, "----------------------\n");
    ...
    *listbuf = 0;
    ...
    for (i = 0; i < nPotential; ++i)
    {
        ...
        if (FS_SV_FOpenFileRead(descPath, &descHandle) > 0 && descHandle)
        {
            ...
            Com_Memset((uint *)descPath, 0, 256);
