...
void __cdecl MemFile_EndSegment(MemoryFile* memFile)
{
    ...
    if (!memFile->memoryOverflow)
    {
        ...
        LABEL_30:
            ...
            memFile->segmentIndex = -1;
            *(uint*)&memFile->buffer[memFile->segmentStart] = memFile->bytesUsed - memFile->segmentStart;
        }
        else
        {
...
uint __cdecl MemFile_deflateEnd(bool compress)
{
...
{
    uint moveByte; // [esp+0h] [ebp-20h]
    uint nextByte; // [esp+8h] [ebp-18h]
