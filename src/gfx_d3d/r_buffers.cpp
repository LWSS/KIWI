...
void *__cdecl R_AllocDynamicIndexBuffer(IDirect3DIndexBuffer9 **ib, uint sizeInBytes)
{
    const char *v3; // eax
    const char *v4; // eax
    int hr; // [esp+0h] [ebp-4h]
...
void *__cdecl R_AllocStaticIndexBuffer(IDirect3DIndexBuffer9 **ib, int sizeInBytes)
{
    ...
    if (!r_loadForRenderer->current.enabled)
        return 0;
    //if (((int(__thiscall *)(IDirect3DDevice9 *, IDirect3DDevice9 *, int, int, int, uint, IDirect3DIndexBuffer9 **, uint))dx.device->CreateIndexBuffer)(
    //    dx.device,
    //    dx.device,
    //    sizeInBytes,
    //    8,
...
void __cdecl R_CreateWorldVertexBuffer(IDirect3DVertexBuffer9 **vb, int *srcData, uint sizeInBytes)
{
