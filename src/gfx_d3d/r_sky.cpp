...
uint __cdecl R_GetSunDvarCount()
{
    return 21;
}
...
void __cdecl R_SetSunFromDvars(sunflare_t *sun)
{
    ...
    sun->spriteMaterial = Material_RegisterHandle(r_sunsprite_shader->current.string, IMAGE_TRACK_FX);
    sun->spriteSize = r_sunsprite_size->current.value;
    sun->flareMaterial = Material_RegisterHandle(r_sunflare_shader->current.string, IMAGE_TRACK_FX);
    ...
    sun->flareMaxDot = cos(DEG2RAD( r_sunflare_max_angle->current.value ));
    ...
    sun->blindMinDot = cos(DEG2RAD( r_sunblind_min_angle->current.value ));
    sun->blindMaxDot = cos(DEG2RAD( r_sunblind_max_angle->current.value ));
    ...
    sun->glareMinDot = cos(DEG2RAD( r_sunglare_min_angle->current.value ));
    sun->glareMaxDot = cos(DEG2RAD( r_sunglare_max_angle->current.value ));
