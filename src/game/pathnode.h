...
enum nearestNodeHeightCheck : int
{
...
void __cdecl Scr_SetPathnodeField(uint entnum, uint offset);
void __cdecl Scr_GetPathnodeField(uint entnum, uint offset);
...
pathnode_t *__cdecl Scr_GetPathnode(uint index);
void __cdecl G_FreePathnodesScriptInfo();
bool __cdecl Path_CompareNodesIncreasing(const pathsort_t &ps1, const pathsort_t &ps2);
uint __cdecl Path_ConvertNodeToIndex(const pathnode_t *node);
pathnode_t *__cdecl Path_ConvertIndexToNode(uint index);
uint __cdecl Path_NodeCount();
...
bool __cdecl Path_IsBadPlaceLink(uint nodeNumFrom, uint nodeNumTo, team_t eTeam);
uint Path_InitLinkCounts();
...
bool __cdecl Path_IsNodeIndex(const pathnode_t *node, uint nodeIndexToCheck);
...
void __cdecl Path_UpdateBestChainNode(pathnode_t *node, pathnode_t **bestNode, uint *foundCount);
...
pathnode_t *__cdecl Path_LoadNode(uint index);
...
void __cdecl GScr_SetDynamicPathnodeField(pathnode_t *node, uint index);
