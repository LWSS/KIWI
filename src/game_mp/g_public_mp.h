...
void __cdecl Scr_GetObjectField(uint classnum, int entnum, int offset);
...
int __cdecl Scr_SetObjectField(uint classnum, uint entnum, uint offset);
int __cdecl Scr_SetEntityField(uint entnum, uint offset);
void __cdecl Scr_GetEntityField(uint entnum, uint offset);
...
gentity_s *__cdecl Scr_GetEntityAllowNull(uint index);
gentity_s *__cdecl Scr_GetEntity(uint index);
...
void __cdecl Scr_Notify(gentity_s *ent, uint16_t stringValue, uint paramcount);
...
uint __cdecl G_SetEntityScriptVariableInternal(const char *key, char *value);
...
void __cdecl G_CheckHitTriggerDamage(gentity_s *pActivator, float *vStart, float *vEnd, int iDamage, uint iMOD);
...
// g_vehicles_mp
uint16_t __cdecl BG_VehiclesGetSlotTagName(int slotIndex);
...
bool __cdecl CG_VehEntityUsingVehicle(int localClientNum, uint entNum);
clientInfo_t *__cdecl ClientInfoForEntity(int localClientNum, uint entNum);
int __cdecl CG_VehLocalClientVehicleSlot(int localClientNum);
int __cdecl CG_VehPlayerVehicleSlot(int localClientNum, uint entNum);
...
int __cdecl VEH_ParseSpecificField(uint8_t *pStruct, const char *pValue, int fieldType);
...
bool __cdecl G_VehImmuneToDamage(gentity_s *ent, int mod, char damageFlags, uint weapon);
