...
int g_block_mem_type[9] = {0, 1, 1, 2, 1, 1, 2, 2, 2};

const char *g_block_mem_name[9] = {"temp",     "runtime", "large_runtime", "physical_runtime", "virtual", "large",
                                   "physical", "vertex",  "index"};
...
void __cdecl DB_RecoverGeometryBuffers(XZoneMemory *zoneMem)
{
    ...
    if (zoneMem->blocks[7].size)
    {
        ...
        lockedData = (uint8_t *)R_AllocStaticVertexBuffer((IDirect3DVertexBuffer9 **)&zoneMem->vertexBuffer,
    ...
    if (zoneMem->blocks[8].size)
    {
        ...
        lockedDataa = (uint8_t *)R_AllocStaticIndexBuffer((IDirect3DIndexBuffer9 **)&zoneMem->indexBuffer,
...
void __cdecl DB_AllocXZoneMemory(uint *blockSize, const char *filename, XZoneMemory *zoneMem, uint allocType)
{
    ...
    for (blockIndex = 0; blockIndex < 9; ++blockIndex)
    {
        ...
        {
            Com_Error(ERR_DROP, "Native fastfile block exceeds allocator limit: %s", filename);
        }
        if (size)
        {
            ...
            if (!buf)
            {
                ...
                Com_Error(ERR_DROP,
                          ...
                          (double)size * 0.00000095367431640625, g_block_mem_name[blockIndex], filename,
    ...
    if (zoneMem->blocks[7].size)
    {
        zoneMem->lockedVertexData = (uint8_t *)R_AllocStaticVertexBuffer(
            (IDirect3DVertexBuffer9 **)&zoneMem->vertexBuffer, zoneMem->blocks[7].size);
    }
    iassert(zoneMem->indexBuffer == NULL);
    iassert(zoneMem->lockedIndexData == NULL);
    if (zoneMem->blocks[8].size)
    {
        zoneMem->lockedIndexData = (uint8_t *)R_AllocStaticIndexBuffer((IDirect3DIndexBuffer9 **)&zoneMem->indexBuffer,
                                                                       zoneMem->blocks[8].size);
    }
}
