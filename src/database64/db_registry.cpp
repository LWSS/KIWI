...
#include "database.h"
#include "db_load_error.h"
#include "db_memory_stream.h"
...
// Native streamed audio is stored as RawFile assets alongside scripts/configs.
#define POOLSIZE_RAWFILE 4096
...
static void __cdecl DB_InitSingleton(void *pool, int size);
...
static void __cdecl DB_DynamicCloneMenu(XAssetHeader from, XAssetHeader to, int swag = 0);
...
static void __cdecl DB_Sleep(uint msec);
static void __cdecl DB_LogMissingAsset(XAssetType type, const char *name);
static void __cdecl DB_RegisteredReorderAsset(int type, const char *assetName, XAssetEntry *assetEntry);
static XAssetEntryPoolEntry *__cdecl DB_FindXAssetEntry(XAssetType type, const char *name);
static uint __cdecl DB_HashForName(const char *name, XAssetType type);
...
static void __cdecl DB_PrintAssetName(XAssetHeader header, int *data);
...
static void __cdecl PrintWaitedError(XAssetType type, const char *name, int waitedMsec);
static bool __cdecl DB_GetInitializing();
static XAssetHeader __cdecl DB_AddXAsset(XAssetType type, XAssetHeader header);
static XAssetEntryPoolEntry *__cdecl DB_LinkXAssetEntry(XAssetEntryPoolEntry *newEntry, int allowOverride);
...
static void __cdecl DB_DynamicCloneXAsset(XAssetHeader from, XAssetHeader to, XAssetType type, int fromDefault);
static void __cdecl DB_DelayedCloneXAsset(XAssetEntry *newEntry);
static bool __cdecl DB_OverrideAsset(uint newZoneIndex, uint existingZoneIndex);
...
static void __cdecl DB_LoadXZone(XZoneInfo *zoneInfo, uint zoneCount);
static void __cdecl DB_LoadZone_f();
static void __cdecl DB_Thread(uint threadContext);
static void DB_TryLoadXFile();
static int __cdecl DB_TryLoadXFileInternal(char *zoneName, int zoneFlags);
static void __cdecl DB_BuildOSPath(const char *zoneName, uint size, char *filename);
static int __cdecl DB_GetZoneAllocType(int zoneFlags);
static void __cdecl DB_UnloadXZone(uint zoneIndex, bool createDefault);
...
static void __cdecl DB_UnloadXAssetsMemoryForZone(int zoneFreeFlags, int zoneFreeBit);
static void __cdecl DB_UnloadXAssetsMemory(XZone *zone, int sortedIndex);
...
static int __cdecl DB_GetAllXAssetOfType_LoadObj(XAssetType type, XAssetHeader *assets, int maxCount);
...
template <typename T> static XAssetHeader DB_AllocTypedAsset(void *arg)
{
    XAssetPoolEntry<T> **head = (XAssetPoolEntry<T> **)arg;
    XAssetPoolEntry<T> *entry = *head;
    if (!entry)
    {
        return XAssetHeader();
    }
    *head = entry->next;
    return XAssetHeader(&entry->entry);
}

template <typename T> static void DB_FreeTypedAsset(void *arg, XAssetHeader asset)
{
    XAssetPoolEntry<T> **head = (XAssetPoolEntry<T> **)arg;
    XAssetPoolEntry<T> *entry = (XAssetPoolEntry<T> *)asset.data;
    entry->next = *head;
    *head = entry;
}

XAssetHeader(__cdecl *DB_AllocXAssetHeaderHandler[ASSET_TYPE_COUNT])(void *) = {
    DB_AllocTypedAsset<XModelPieces>,
    DB_AllocTypedAsset<PhysPreset>,
    DB_AllocTypedAsset<XAnimParts>,
    DB_AllocTypedAsset<XModel>,
    &DB_AllocMaterial,
    DB_AllocTypedAsset<MaterialTechniqueSet>,
    DB_AllocTypedAsset<GfxImage>,
    DB_AllocTypedAsset<snd_alias_list_t>,
    DB_AllocTypedAsset<SndCurve>,
    DB_AllocTypedAsset<LoadedSound>,
    &node1_,
    &node1_,
    &node1_,
    &node1_,
    &node1_,
    DB_AllocTypedAsset<MapEnts>,
    &node1_,
    DB_AllocTypedAsset<GfxLightDef>,
    NULL,
    DB_AllocTypedAsset<Font_s>,
    DB_AllocTypedAsset<MenuList>,
    DB_AllocTypedAsset<menuDef_t>,
    DB_AllocTypedAsset<LocalizeEntry>,
    DB_AllocTypedAsset<WeaponDef>,
    DB_AllocTypedAsset<SndDriverGlobals>,
    DB_AllocTypedAsset<FxEffectDef>,
    DB_AllocTypedAsset<FxImpactTable>,
    NULL,
    NULL,
    NULL,
    NULL,
    DB_AllocTypedAsset<RawFile>,
    DB_AllocTypedAsset<StringTable>}; // idb
...
void(__cdecl *DB_FreeXAssetHeaderHandler[ASSET_TYPE_COUNT])(void *,
                                                            XAssetHeader) = {DB_FreeTypedAsset<XModelPieces>,
                                                                             DB_FreeTypedAsset<PhysPreset>,
                                                                             DB_FreeTypedAsset<XAnimParts>,
                                                                             DB_FreeTypedAsset<XModel>,
                                                                             DB_FreeMaterial,
                                                                             DB_FreeTypedAsset<MaterialTechniqueSet>,
                                                                             DB_FreeTypedAsset<GfxImage>,
                                                                             DB_FreeTypedAsset<snd_alias_list_t>,
                                                                             DB_FreeTypedAsset<SndCurve>,
                                                                             DB_FreeTypedAsset<LoadedSound>,
                                                                             NULLSUB,
                                                                             NULLSUB,
                                                                             NULLSUB,
                                                                             NULLSUB,
                                                                             NULLSUB,
                                                                             DB_FreeTypedAsset<MapEnts>,
                                                                             NULLSUB,
                                                                             DB_FreeTypedAsset<GfxLightDef>,
                                                                             NULL,
                                                                             DB_FreeTypedAsset<Font_s>,
                                                                             DB_FreeTypedAsset<MenuList>,
                                                                             DB_FreeTypedAsset<menuDef_t>,
                                                                             DB_FreeTypedAsset<LocalizeEntry>,
                                                                             DB_FreeTypedAsset<WeaponDef>,
                                                                             DB_FreeTypedAsset<SndDriverGlobals>,
                                                                             DB_FreeTypedAsset<FxEffectDef>,
                                                                             DB_FreeTypedAsset<FxImpactTable>,
                                                                             NULL,
                                                                             NULL,
                                                                             NULL,
                                                                             NULL,
                                                                             DB_FreeTypedAsset<RawFile>,
                                                                             DB_FreeTypedAsset<StringTable>}; // idb
...
const char *g_defaultAssetName[ASSET_TYPE_COUNT] = {"",
                                                    ...
                                                    "",
                                                    "",
                                                    "mp/defaultStringTable.csv"}; // idb

const char *g_assetNames[ASSET_TYPE_COUNT] = // SP/MP same
    {"xmodelpieces", "physpreset", "xanim",        "xmodel",           "material",   "techset",  "image",
     "sound",        "sndcurve",   "loaded_sound", "col_map_sp",       "col_map_mp", "com_map",  "game_map_sp",
     "game_map_mp",  "map_ents",   "gfx_map",      "lightdef",         "ui_map",     "font",     "menufile",
     "menu",         "localize",   "weapon",       "snddriverglobals", "fx",         "impactfx", "aitype",
...
// uint volatile g_mainThreadBlocked  829f278c     db_registry.obj
...
static XAssetPool<SndDriverGlobals, POOLSIZE_SNDDRIVER_GLOBALS> g_SndDriverGlobalsPool;
...
volatile uint g_zoneInfoCount;
bool g_initializing;

char g_debugZoneName[64];
uint g_zoneAllocType;
uint g_zoneIndex;
uint _S1;
...
void __cdecl Hunk_OverrideDataForFile(int type, const char *name, void *data)
{
    ...
    for (searchFileData = com_fileDataHashTable[FS_HashFileName(name, 1024)]; searchFileData;
...
template <typename T> void __cdecl DB_InitPool(void *arg, int size)
{
...
void *DB_XAssetPool[ASSET_TYPE_COUNT] = {&g_XModelPiecesPool,
                                         &g_PhysPresetPool,
                                         &g_XAnimPartsPool,
                                         &g_XModelPool,
                                         &g_MaterialPool,
...
void __cdecl DB_GetIndexBufferAndBase(uint8_t zoneHandle, void *indices, void **ib, int *baseIndex)
{
    ...
    *baseIndex = (int)(((uint8_t *)indices - g_zones[zoneHandle].mem.blocks[8].data) / sizeof(uint16_t));
}
...
void __cdecl DB_BuildOSPath_Mod(const char *zoneName, uint size, char *filename)
{
    char *v3;           // eax
...
bool __cdecl DB_ModFileExists()
{
    ...
    zoneFile = CreateFileA(filename, 0x80000000, 1u, 0, 3u, FILE_FLAG_OVERLAPPED, 0);
    if (zoneFile == (void *)-1)
    {
        return 0;
    }
...
void __cdecl DB_EndRecoverLostDevice()
{
    ...
    {
        MyAssertHandler("c:\\trees\\cod3\\src\\gfx_d3d\\../qcommon/threads_interlock.h", 76, 0, "%s",
...
{
    *header = DB_AddXAsset(ASSET_TYPE_XMODELPIECES, *header);
}

void Mark_XModelPiecesAsset(XModelPieces *pieces)
{
    XAssetHeader header = {};
    header.xmodelPieces = pieces;
    DB_GetXAsset(ASSET_TYPE_XMODELPIECES, header);
}
...
void __cdecl Load_MaterialTechniqueSetAsset(XAssetHeader *techniqueSet)
{
    techniqueSet->xmodelPieces =
        DB_AddXAsset(ASSET_TYPE_TECHNIQUE_SET, (XAssetHeader)techniqueSet->xmodelPieces).xmodelPieces;
...
{
    gameWorldSp->xmodelPieces =
        DB_AddXAsset(ASSET_TYPE_GAMEWORLD_SP, (XAssetHeader)gameWorldSp->xmodelPieces).xmodelPieces;
}
...
{
    gameWorldMp->xmodelPieces =
        DB_AddXAsset(ASSET_TYPE_GAMEWORLD_MP, (XAssetHeader)gameWorldMp->xmodelPieces).xmodelPieces;
}
...
void __cdecl Load_MenuAsset(XAssetHeader *menu)
{
    ...
    //     *(XAssetHeader *)(*(uint *)(header.xmodelPieces[23].numpieces + 4 * i) + 232) =
    //     (XAssetHeader)menu->xmodelPieces;
}
...
{
    *weapon = DB_AddXAsset(ASSET_TYPE_WEAPON, *weapon);
}
...
{
    *fx = DB_AddXAsset(ASSET_TYPE_FX, *fx);
}

void Load_SndDriverGlobalsAsset(XAssetHeader *header)
{
    *header = DB_AddXAsset(ASSET_TYPE_SNDDRIVER_GLOBALS, *header);
}

void Mark_SndDriverGlobalsAsset(SndDriverGlobals *globals)
{
    XAssetHeader header = {};
    header.sndDriverGlobals = globals;
    DB_GetXAsset(ASSET_TYPE_SNDDRIVER_GLOBALS, header);
}

DB64MemoryStream *DB64_OpenRawFileStream(const char *name)
{
    InterlockedIncrement(&db_hashCritSect.readCount);
    while (db_hashCritSect.writeCount)
    {
    ...
    XAssetEntryPoolEntry *entry = DB_FindXAssetEntry(ASSET_TYPE_RAWFILE, name);
    DB64MemoryStream *stream = NULL;
    if (entry && entry->entry.zoneIndex)
    {
        const RawFile *raw = entry->entry.asset.header.rawfile;
        stream = DB64_CreateMemoryStream(raw->buffer, raw->len);
    }
    InterlockedDecrement(&db_hashCritSect.readCount);
    return stream;
}
...
const FxEffectDef *DB64_FindLoadedEffect(const char *name)
{
    ...
    XAssetEntryPoolEntry *entry = DB_FindXAssetEntry(ASSET_TYPE_FX, name);
    const FxEffectDef *effect = NULL;
    if (entry && entry->entry.zoneIndex)
    {
        effect = entry->entry.asset.header.fx;
        entry->entry.inuse = 1;
    }
    InterlockedDecrement(&db_hashCritSect.readCount);
    return effect;
...
{
    *impactFx = DB_AddXAsset(ASSET_TYPE_IMPACT_FX, *impactFx);
}
...
{
    stringTable->xmodelPieces =
        DB_AddXAsset(ASSET_TYPE_STRINGTABLE, (XAssetHeader)stringTable->xmodelPieces).xmodelPieces;
}
...
{
    Material_DirtySort();
    return DB_AllocTypedAsset<Material>(arg);
}

void __cdecl DB_FreeMaterial(void *arg, XAssetHeader header)
{
    Material_DirtySort();
    DB_FreeTypedAsset<Material>(arg, header);
}

void __cdecl DB_EnumXAssets_FastFile(XAssetType type, void(__cdecl *func)(XAssetHeader, void *), void *inData,
                                     bool includeOverride)
{
    uint hash;                        // [esp+4h] [ebp-14h]
    uint assetEntryIndex;             // [esp+8h] [ebp-10h]
    XAssetEntryPoolEntry *assetEntry; // [esp+10h] [ebp-8h]
    uint overrideAssetEntryIndex;     // [esp+14h] [ebp-4h]
    ...
    for (hash = 0; hash < 0x8000; ++hash)
    {
        for (assetEntryIndex = db_hashTable[hash]; assetEntryIndex; assetEntryIndex = assetEntry->entry.nextHash)
        {
            ...
            if (assetEntry->entry.asset.type == type)
            {
                ...
                if (includeOverride)
                {
                    for (overrideAssetEntryIndex = assetEntry->entry.nextOverride; overrideAssetEntryIndex;
...
void __cdecl DB_EnumXAssets(XAssetType type, void(__cdecl *func)(XAssetHeader, void *), void *inData,
                            bool includeOverride)
{
    if (IsFastFileLoad())
    {
...
void __cdecl DB_EnumXAssetsFor(fileData_s *fileData, int fileDataType, void(__cdecl *func)(void *, void *),
                               void *inData)
{
    while (fileData)
    {
...
void __cdecl DB_Sleep(uint msec)
{
...
XAssetEntryPoolEntry *__cdecl DB_FindXAssetEntry(XAssetType type, const char *name)
{
    ...
    for (assetEntryIndex = db_hashTable[DB_HashForName(name, type)]; assetEntryIndex;
...
void DB_EndReorderZone()
{
    ...
    if (s_dbReorder.entryCount)
    {
        ...
        if (file != (HANDLE)-1)
        {
            ...
            // std::_Sort<DBReorderAssetEntry *, int, bool(__cdecl *)(DBReorderAssetEntry const &, DBReorderAssetEntry
            // const &)>(
            //     (GfxSModelSurfStats *)s_dbReorder.entries,
            //     (GfxSModelSurfStats *)&s_dbReorder.entries[s_dbReorder.entryCount],
            //     (int)(16 * s_dbReorder.entryCount) >> 4,
            ...
            for (entryIter = 0; entryIter < s_dbReorder.entryCount; ++entryIter)
            {
                ...
                else
                {
                    ...
                    {
                        bytes = Com_sprintf(line, 0x200u, "%s,%s,%s,%s\r\n", entry->typeString, entry->assetName,
                                            s_dbReorder.zoneName, "all_mp");
                    }
                    else
                    {
...
void __cdecl DB_RegisteredReorderAsset(int type, const char *assetName, XAssetEntry *assetEntry)
{
    ...
    if (s_dbReorder.entryCount)
    {
        if (type == ASSET_TYPE_LOCALIZE_ENTRY)
        {
            ...
            {
                s_dbReorder.loadedLocalization =
                    DB_RegisterAllReorderAssetsOfType(ASSET_TYPE_LOCALIZE_ENTRY, assetEntry) != 0;
            }
        }
        ...
        else if (!s_dbReorder.lastEntry || s_dbReorder.lastEntry->type != type ||
...
uint __cdecl DB_HashForName(const char *name, XAssetType type)
{
    int c; // [esp+8h] [ebp-4h]
...
XAssetEntry *__cdecl DB_CreateDefaultEntry(XAssetType type, char *name)
{
    ...
    if (!asset.header.data)
    {
        ...
        else
        {
            Com_Error(ERR_DROP, "Could not load default asset '%s' for asset type '%s'.\nTried to load asset '%s'.",
                      g_defaultAssetName[type], g_assetNames[type], name);
        }
    }
    asset.type = type;
    ++g_defaultAssetCount;
    ...
    db_hashTable[DB_HashForName(name, type)] = (uint16_t)((XAssetEntryPoolEntry *)newEntry - g_assetEntryPool);
...
{
    const char *XAssetName;           // eax
    uint assetEntryIndex;             // [esp+8h] [ebp-Ch]
...
bool __cdecl DB_IsXAssetDefault(XAssetType type, const char *name)
{
    const char *XAssetName;           // eax
    uint hash;                        // [esp+4h] [ebp-Ch]
    uint assetEntryIndex;             // [esp+8h] [ebp-8h]
...
XAssetEntryPoolEntry *__cdecl DB_LinkXAssetEntry(XAssetEntryPoolEntry *newEntry, int allowOverride)
{
    ...
    uint hash;                                // [esp+4h] [ebp-8A8h]
    uint existingEntryIndex;                  // [esp+8h] [ebp-8A4h]
    ...
    hash = DB_HashForName(name, type);
    existingEntry = NULL;
    for (existingEntryIndex = db_hashTable[hash]; existingEntryIndex;
    ...
    if (existingEntry->entry.zoneIndex)
    {
        ...
        if (!*g_defaultAssetName[type] && type != ASSET_TYPE_RAWFILE && type != ASSET_TYPE_MAP_ENTS)
        {
            ...
            Com_Error(ERR_DROP, "Attempting to override asset '%s' from zone '%s' with zone '%s'", name,
                      g_zones[existingEntry->entry.zoneIndex].name, g_zones[newEntry->entry.zoneIndex].name);
        }
        if (!DB_OverrideAsset(newEntry->entry.zoneIndex, existingEntry->entry.zoneIndex))
        {
            for (pOverrideAssetEntryIndex = &existingEntry->entry.nextOverride; *pOverrideAssetEntryIndex;
    ...
    else
    {
        if (g_copyInfoCount >= 0x800)
        {
            Com_Printf(CON_CHANNEL_DONT_FILTER,
                       "g_copyInfo exceeded\n"); // LWSS: if this hits, it means that PostLoadXZone isn't setting back
                                                 // to zero (prob not being called)
            for (i = 0; i < 0x800; ++i)
            {
...
{
    uint assetEntryIndex;    // [esp+4h] [ebp-14h]
...
static thread_local jmp_buf *s_loadErrorTarget;
static char s_loadError[4096];
static uint s_failedZone;

void DB64_CaptureLoadError(const char *format, va_list arguments)
{
    if (s_loadErrorTarget)
    {
        vsnprintf(s_loadError, sizeof(s_loadError), format, arguments);
        longjmp(*s_loadErrorTarget, 1);
    }
}

static void DB64_FinishFailedLoad(bool releaseAssets = false)
{
    if (s_loadError[0])
    {
        if (Sys_IsMainThread())
        {
            char message[sizeof(s_loadError)];
            memcpy(message, s_loadError, sizeof(message));
            s_loadError[0] = 0;
            Sys_DatabaseCompleted2();
            // Com_Error shuts down game users before Com_Restart releases assets.
    ...
    // Com_Restart has shut down collision and all other game users.
    if (s_failedZone)
    {
        Material_ClearShaderUploadList();
        Sys_LockWrite(&db_hashCritSect);
        for (uint i = 0; i < g_copyInfoCount;)
        {
            if (g_copyInfo[i]->zoneIndex == s_failedZone)
            {
                DB_RemoveXAsset(&g_copyInfo[i]->asset);
                DB_FreeXAssetEntry((XAssetEntryPoolEntry *)g_copyInfo[i]);
                g_copyInfo[i] = g_copyInfo[--g_copyInfoCount];
            }
            else
            {
                ++i;
        ...
        DB_UnloadXZone(s_failedZone, false);
        DB_FreeUnusedResources();
        DB_UnloadXZoneMemory(&g_zones[s_failedZone]);
        --g_zoneCount; // The failed zone was the last one claimed; later requests were cancelled.
        g_zoneHandles[g_zoneCount] = 0;
        s_failedZone = 0;
        Sys_UnlockWrite(&db_hashCritSect);
    }
}
...
void DB_PostLoadXZone()
{
    ...
    if (!Sys_IsDatabaseReady2())
    {
        if (g_copyInfoCount)
        {
            ...
            if (!Sys_IsMainThread() || (++g_mainThreadBlocked, remoteScreenUpdateNesting = R_PopRemoteScreenUpdate(),
                                        --g_mainThreadBlocked, g_copyInfoCount))
...
{
    uint j;            // [esp+4h] [ebp-14h]
    uint ja;           // [esp+4h] [ebp-14h]
    bool unloadedZone; // [esp+Bh] [ebp-Dh]
    uint zoneIndex;    // [esp+Ch] [ebp-Ch]
...
void __cdecl DB_InitPoolHeader(XAssetType type)
{
    ...
    {
        if (!DB_InitPoolHeaderHandler[type] || !DB_AllocXAssetHeaderHandler[type] ||
            !DB_FreeXAssetHeaderHandler[type])
        {
            Com_Error(ERR_FATAL, "Incomplete asset pool handlers for '%s'", g_assetNames[type]);
            return;
        }
...
void __cdecl DB_LoadXZone(XZoneInfo *zoneInfo, uint zoneCount)
{
    uint j;             // [esp+0h] [ebp-Ch]
    char *zoneName;     // [esp+4h] [ebp-8h]
    uint zoneInfoCount; // [esp+8h] [ebp-4h]
    ...
    for (j = 0; j < zoneCount; ++j)
    {
        ...
        if (zoneName)
        {
            if (zoneInfoCount >= 8)
            {
                MyAssertHandler(".\\database\\db_registry.cpp", 3249, 0, "%s",
...
{
    if (!Sys_SpawnDatabaseThread((void(__cdecl *)(uint))DB_Thread))
    {
        Sys_Error("Failed to create database thread");
    }
}

void __cdecl DB_Thread(uint threadContext)
{
    jmp_buf *Value; // eax
...
void DB_TryLoadXFile()
{
    ...
    uint zoneInfoCount; // [esp+4h] [ebp-4h]
...
void __cdecl DB_AddReorderAsset(const char *typeString, const char *assetName)
{
    ...
    if (type == ASSET_TYPE_WEAPON)
    {
        ...
        {
            MyAssertHandler(".\\database\\db_registry.cpp", 1911, 0, "%s",
...
char __cdecl DB_ShouldLoadFromModDir(const char *zoneName)
{
    ...
    {
        MyAssertHandler(".\\database\\db_registry.cpp", 3543, 0, "%s",
...
int __cdecl DB_TryLoadXFileInternal(char *zoneName, int zoneFlags)
{
    ...
    {
        zoneFile = CreateFileA("update:\\mp_patch.ff", 0x80000000, 0, 0, 3u, FILE_FLAG_OVERLAPPED, 0);
        if (zoneFile == (void *)-1)
        {
            Com_Printf(CON_CHANNEL_SYSTEM, "Loading mp_patch.ff from disc, not from the update drive\n");
            DB_BuildOSPath(zoneName, 256, filename);
            zoneFile = CreateFileA(filename, 0x80000000, 0, 0, 3u, FILE_FLAG_OVERLAPPED, 0);
            if (zoneFile == (void *)-1)
            {
    ...
    if (zoneFile == (void *)-1)
    {
        ...
        {
            snprintf(s_loadError, sizeof(s_loadError), "Could not find zone '%s'", filename);
        }
        return 0;
    }
    else
    {
        ...
        {
            MyAssertHandler("c:\\trees\\cod3\\src\\qcommon\\../universal/assertive.h", 281, 0,
                            "i == static_cast< Type >( i )\n\t%i, %i", g_zoneIndex, (uint8_t)g_zoneIndex);
        }
        ...
        {
            _S1 |= 1u;
            zone_reorder = Dvar_RegisterString("zone_reorder", "", DVAR_NOFLAG,
        ...
        iassert(zone);
        jmp_buf loadErrorTarget;
        if (setjmp(loadErrorTarget))
        {
            s_loadErrorTarget = NULL;
            DB_CloseNativeFile();
            PMem_EndAlloc(zone->name, g_zoneAllocType);
            s_failedZone = g_zoneIndex;
            g_loadingZone = 0;
            g_mayRecoverLostAssets = 1;
            return 0;
        }
        s_loadErrorTarget = &loadErrorTarget;
...
void __cdecl DB_BuildOSPath(const char *zoneName, uint size, char *filename)
{
    char *v3;       // eax
...
void __cdecl DB_UnloadXZone(uint zoneIndex, bool createDefault)
{
    uint hash;                                // [esp+4h] [ebp-28h]
    ...
    uint overrideAssetEntryIndex;             // [esp+28h] [ebp-4h]
...
{
    uint nextAssetEntryIndex;         // [esp+0h] [ebp-10h]
    uint hash;                        // [esp+4h] [ebp-Ch]
    uint assetEntryIndex;             // [esp+8h] [ebp-8h]
    XAssetEntryPoolEntry *assetEntry; // [esp+Ch] [ebp-4h]
...
{
    uint hash;                        // [esp+0h] [ebp-18h]
    uint hasha;                       // [esp+0h] [ebp-18h]
