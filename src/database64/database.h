...
void __cdecl DB_EnumXAssetsFor(fileData_s *fileData, int fileDataType, void (*func)(void *, void *), void *inData);
...
void __stdcall DB_FileReadCompletion(uint dwErrorCode, uint dwNumberOfBytesTransfered, _OVERLAPPED *lpOverlapped);
void __cdecl DB_LoadXFileInternal();
void __cdecl DB_ResetZoneSize(int trackLoadProgress);
void __cdecl DB_LoadXFile(const char *path, void *f, const char *filename, XZoneMemory *zoneMem,
                          void(__cdecl *interrupt)(), uint8_t *buf, int allocType);

// db_memory
void __cdecl DB_RecoverGeometryBuffers(XZoneMemory *zoneMem);
void __cdecl DB_ReleaseGeometryBuffers(XZoneMemory *zoneMem);
void __cdecl DB_AllocXZoneMemory(uint *blockSize, const char *filename, XZoneMemory *zoneMem, uint allocType);
...
// db_stream_load
void __cdecl Load_Stream(bool atStreamStart, uint8_t *ptr, size_t size);
void __cdecl Load_DelayStream();
void __cdecl DB_ConvertOffsetToAlias(uintptr_t *data);
void __cdecl DB_ConvertOffsetToPointer(uintptr_t *data);
void __cdecl Load_XStringCustom(char **str);
void DB64_LoadAssetString(const char **str);
void DB64_ConvertOffsetRange(uintptr_t *pointer, size_t size);
