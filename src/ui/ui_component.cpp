...
void Scr_ScriptWatch::AddElement(Scr_WatchElement_s *element, char *text)
{
    ...
    if (!element->breakpoint)
    {
        ...
        if (scriptExpr.parseData.node[0].type != 83)
        {
            if (v3 > 83 && v3 <= 85)
            {
                Scr_FreeDebugExpr(&scriptExpr);
        ...
        Scr_ExecCode(scriptExpr.parseData.node[1].codePosValue, this->localId);
...
{
    uint bufferIndex; // [esp+4h] [ebp-10h]
    uint lineNum; // [esp+8h] [ebp-Ch]
    const char *codePos; // [esp+Ch] [ebp-8h]
    uint sourcePos; // [esp+10h] [ebp-4h]

    iassert(!Sys_IsRemoteDebugClient());
    if (scrVarPub.evaluate && (element->objectType == VAR_DEAD_THREAD || element->objectType == VAR_THREAD && element->directObject))
    {
...
{
    uint LineNum; // eax
    uint bufferIndex; // [esp+4h] [ebp-8h]
    ...
    float compY)
{
    ...
    uint bufferIndex; // [esp+58h] [ebp-B8h]
...
bool Scr_OpenScriptList::SetSelectedLineFocus(
    ...
    bool user)
{
    uint sortedIndex; // [esp+4h] [ebp-Ch]
    uint bufferIndex; // [esp+8h] [ebp-8h]
...
void Scr_ScriptWatch::PasteBreakpointElement(
    ...
    bool user)
{
    uint bufferIndex; // [esp+4h] [ebp-14h]
...
{
    uint AllVariableField_DONE; // eax
    const char *v3; // eax
    const char *v4; // eax
    ...
    int(__cdecl * v10)(uint *, uint *); // [esp+14h] [ebp-11Ch]
    ...
    uint hardcodedNames[5]; // [esp+BCh] [ebp-74h] BYREF
    Scr_WatchElement_s *childElement; // [esp+D0h] [ebp-60h]
    uint newIndex; // [esp+D4h] [ebp-5Ch]
    uint oldChildCount; // [esp+D8h] [ebp-58h]
    int(__cdecl * compare)(uint*, uint*); // [esp+DCh] [ebp-54h]
    ...
    uint threadId; // [esp+10Ch] [ebp-24h]
    uint oldIndex; // [esp+110h] [ebp-20h]
    uint *names; // [esp+114h] [ebp-1Ch]
    bool hardcodedField; // [esp+11Bh] [ebp-15h]
    uint nameIndex; // [esp+11Ch] [ebp-14h]
    uint count; // [esp+120h] [ebp-10h]
...
void Scr_AbstractScriptList::DeleteEntryInternal()
{
    if (this->selectedLine >= 0)
    {
        memmove(
            ...
            sizeof(Scr_ScriptWindow *) * (this->numLines - 1 - this->selectedLine));
        --this->numLines;
        UI_LinesComponent::UpdateHeight();
    }
}
...
void Scr_AbstractScriptList::AddEntry(Scr_ScriptWindow *scriptWindow, bool select)
{
    ...
    for (i = 0; i < this->numLines; ++i)
    {
        if (this->scriptWindows[i] == scriptWindow)
        {
            if (selectedLine <= i)
            {
                memmove(&this->scriptWindows[selectedLine + 1], &this->scriptWindows[selectedLine], sizeof(Scr_ScriptWindow *) * (i - selectedLine));
            }
            else
            {
                newIndex = selectedLine - 1;
                memmove(&this->scriptWindows[i], &this->scriptWindows[i + 1], sizeof(Scr_ScriptWindow *) * (selectedLine - 1 - i));
            }
            goto found_0;
        }
    }
    newNumLines = this->numLines + 1;
    newScriptWindows = (byte*)Scr_AllocDebugMem(sizeof(Scr_ScriptWindow *) * newNumLines, "Scr_AbstractScriptList::AddEntry");
    if (this->scriptWindows)
    {
        memcpy(newScriptWindows, this->scriptWindows, sizeof(Scr_ScriptWindow *) * selectedLine);
        memcpy(
            &newScriptWindows[sizeof(Scr_ScriptWindow *) * selectedLine + sizeof(Scr_ScriptWindow *)],
            &this->scriptWindows[selectedLine],
            sizeof(Scr_ScriptWindow *) * (this->numLines - selectedLine));
        Scr_FreeDebugMem(this->scriptWindows);
    }
...
{
    uint startSourcePos; // [esp+4h] [ebp-1Ch]
...
void Scr_ScriptWindow::EnterCallInternal()
{
    ...
    uint startSourcePos; // [esp+1Ch] [ebp-18h] BYREF
    const char *codePos; // [esp+20h] [ebp-14h]
    uint sourcePos; // [esp+24h] [ebp-10h] BYREF
    Scr_Breakpoint **pBreakpoint; // [esp+28h] [ebp-Ch]
    const char *destCodePos; // [esp+2Ch] [ebp-8h]
    uint endSourcePos; // [esp+30h] [ebp-4h] BYREF
