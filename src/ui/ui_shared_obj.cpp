...
void __cdecl NumberValue(char *string, __int16 subtype, uint *intvalue, long double *floatvalue)
{
    uint dotfound; // [esp+40h] [ebp-4h]
...
int __cdecl PC_Script_Parse(int handle, const char **out)
{
    ...
    while (1)
    {
        ...
        if ((uint)(&pc_token.string[strlen(pc_token.string) + 1]
            - &pc_token.string[1]
            + &dst[strlen(dst) + 1]
            - &dst[1]) > 0x1400)
            break;
        ...
        if ((uint)(&dst[strlen(dst) + 1] - &dst[1] + 1) > 0x1400)
            break;
        I_strncat(dst, 5120, " ");
    }
...
bool __cdecl ItemParse_IsValidTextAlignment(uint textAlignMode)
{
