...
#define NOUNCRYPT
/* CoD2 IWD files are not encrypted */
...
/* ========================================================================
 * IW (Infinity Ward) extensions for CoD2 map compiler.
 * These functions are NOT part of stock minizip.
 * ======================================================================== */

/*
 * unzReopen - Opens the same zip file with a new FILE handle,
 * copying central directory state from an existing unzFile.
 * Used by CoD2 FS to share zip state across file handles.
 */
extern unzFile ZEXPORT unzReopen(const char *path, unzFile templateFile)
{
    unz64_s *tmpl = (unz64_s *)templateFile;
    unz64_s *s;
    FILE *fin;
    ...
    /* Copy entire state from template */
    memcpy(s, tmpl, sizeof(unz64_s));
...
/*
 * unzGetCurrentFileInfoPosition - Returns the position of the current
 * file entry in the central directory. Used to save position for later
 * seeking with unzSeekToFile().
 */
extern int ZEXPORT unzGetCurrentFileInfoPosition(unzFile file, ZPOS64_T *pos)
{
    unz64_s *s;
...
/*
 * unzSeekToFile - Seeks to a file in the zip by its central directory
 * position (as returned by unzGetCurrentFileInfoPosition).
 * Re-reads the file info for the new position.
 */
extern int ZEXPORT unzSeekToFile(unzFile file, ZPOS64_T pos)
{
    unz64_s *s;
    int err;
