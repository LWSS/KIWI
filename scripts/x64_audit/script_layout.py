"""Report native script layouts for architecture-specific assertions."""
...
root = Path.cwd()
out = Path(os.environ['TEMP'])/'kiwi-x64-epic/script-layout'
...
headers = sorted((root/'src/script').glob('*.h'))
types = []
for header in headers:
    types += re.findall(r'static_assert\(sizeof\((\w+)\)',header.read_text())
source = '#include <universal/q_shared.h>\n#include <stdio.h>\n'
for header in headers:
    if header.name not in ['scr_readwrite.h','scr_yacc_structs.h']:
        source += '#include <script/'+header.name+'>\n'
source += 'int main(){\n'
